// ─────────────────────────────────────────────────────────────────────────────
// ADMIN SETTLEMENTS — Zakładka "Rozliczenia"
...
const STATUS_LABEL = {
  signed:   { text: "Podpisany",      bg: "#EBF5FB", color: C.blue },
...
export function AdminSettlements({ token }) {
  ...
  const fetchReports = useCallback(async () => {
    ...
    try {
      ...
      if (e.name !== "AbortError") setErr(e.message || "Błąd pobierania raportów.");
    } finally {
      setLoading(false);
    }
  ...
  async function handleDelete(id) {
    ...
    try {
      ...
      showToast("Raport usunięty.", true);
    } catch (e) {
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead>
              <tr style={{ background: C.greyBg, borderBottom: `2px solid ${C.grey}` }}>
                {["Data raportu", "Klient", "Technik", "Dodano", "Status", "Akcje"].map(h => (
                  <th key={h} style={{
