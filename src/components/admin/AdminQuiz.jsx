...
export function AdminQuiz() {
  ...
  async function createQuiz() {
    ...
    if (!title) { setErr(T.adm_quiz_no_name); return; }
  ...
  async function saveQ() {
    if (!fQ.trim()||!fA.trim()||!fB.trim()||!fC.trim()) { setErr(T.adm_quiz_fill_all); return; }
    setSaving(true); setErr("");
  ...
  if (!selQuiz) return (
    <div style={{padding:12,display:"flex",flexDirection:"column",gap:10}}>
      ...
      <button onClick={() => { setShowQuizForm(p => !p); setErr(""); }}
        ...
        {showQuizForm ? T.adm_quiz_cancel : "+ Nowy quiz"}
      </button>
      ...
      {showQuizForm && (
        <div style={{background:C.white,padding:16,borderRadius:8,boxShadow:"0 2px 8px rgba(0,0,0,.1)"}}>
          ...
          <div style={{display:"flex",gap:0,marginBottom:12,border:`1px solid ${C.grey}`,borderRadius:6,overflow:"hidden"}}>
            {[["training",T.adm_quiz_from_train],["custom",T.adm_quiz_custom]].map(([mode,label]) => (
          ...
          <button onClick={createQuiz} disabled={saving}
            ...
            {saving ? "Creating..." : "Utwórz quiz i dodaj pytania →"}
          </button>
        </div>
      )}
  ...
  return (
    <div style={{padding:12,display:"flex",flexDirection:"column",gap:10}}>
      ...
      {showQForm && (
        <div style={{background:C.white,padding:16,borderRadius:8,boxShadow:"0 2px 8px rgba(0,0,0,.1)"}}>
          <div style={{fontSize:13,fontWeight:700,marginBottom:12}}>{editingQ?T.adm_quiz_edit:T.adm_quiz_new}</div>
          <textarea value={fQ} onChange={e => setFQ(e.target.value)} placeholder={T.adm_quiz_body_ph} rows={3}
          ...
          <div style={{display:"flex",gap:8}}>
            <button onClick={saveQ} disabled={saving}
              ...
              {saving?T.adm_quiz_saving:T.adm_quiz_save}
            </button>
        ...
        >
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8,marginBottom:6}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:6,flexShrink:0}}>
              {usedIds.has(q.id) && (
                <span title={T.adm_quiz_used_tip} style={{fontSize:14,lineHeight:1}}>✔️</span>
              )}
