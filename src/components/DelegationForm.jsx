...
// ── Helpers: konwersja dat DD.MM.YYYY <-> YYYY-MM-DD ─────────────────────────
...
// ── Komponent: pole daty DD.MM.RRRR z ikonką kalendarza ─────────────────────
function DateInputDel({ value, onChange, style }) {
  const [disp, setDisp] = useState(isoToDisp(value));
  const hiddenRef = useRef(null);
  useEffect(() => { setDisp(isoToDisp(value)); }, [value]);

  function handleText(e) {
    const masked = applyDateMask(e.target.value);
    setDisp(masked);
    const iso = dispToIso(masked);
    if (iso) onChange(iso);
  }
  function handleBlur() {
    if (!dispToIso(disp)) setDisp(isoToDisp(value));
  }
  function handleCalendar(e) {
    const iso = e.target.value;
    if (iso) { setDisp(isoToDisp(iso)); onChange(iso); }
  }
  function openPicker() {
    const el = hiddenRef.current;
    if (!el) return;
    if (el.showPicker) { try { el.showPicker(); } catch(_) { el.click(); } }
    else el.click();
  }
  ...
  return (
    <div style={{ position:"relative", display:"flex", alignItems:"center" }}>
      ...
      />
      <span onClick={openPicker} title="Wybierz datę"
        style={{ position:"absolute", right:8, cursor:"pointer", fontSize:14, color:"#999", userSelect:"none" }}>
        &#128197;
      </span>
      <input ref={hiddenRef} type="date" value={value||""} onChange={handleCalendar} tabIndex={-1}
...
function calcDuration(depDate, depTime, retDate, retTime) {
  ...
  const hDisp  = h => `${Math.floor(h)}h`;

  // ── Reguły diet (Rozp. MPiPS Dz.U.2013 poz.167) ─────────────────────────
  //
  // PODRÓŻ < 24h (jednorazowa):
  //   poniżej 8h          → brak diety
  //   od 8h do 12h        → ½ diety (22,50 zł)
  //   powyżej 12h         → dieta pełna (45,00 zł)
  //
  // PODRÓŻ WIELODNIOWA (≥ 24h):
  //   każda pełna doba    → dieta pełna (45,00 zł)
  //   niepełna doba końcowa:
  //     do 8h             → ½ diety (22,50 zł)   ← UWAGA: nie "brak"!
  //     powyżej 8h        → dieta pełna (45,00 zł)
  // ─────────────────────────────────────────────────────────────────────────
  let dieta = 0;
  let label = "";
  ...
  if (totalH < 8) {
    ...
    label = `${hDisp(totalH)} — brak diety (podróż poniżej 8h)`;

  } else if (totalH <= 12) {
    // Od 8h do 12h — ½ diety
    dieta = S.dietaHalf;
    label = `${hDisp(totalH)} (½ diety — podróż 8–12h)`;

  } else if (totalH <= 24) {
    // Powyżej 12h, nie przekracza doby — dieta pełna
    dieta = S.dietaFull;
    label = `${hDisp(totalH)} (pełna dieta — podróż powyżej 12h)`;

  } else {
    // Podróż wielodniowa
    const fullDoby = Math.floor(totalH / 24);
    const remH     = totalH - fullDoby * 24;
    dieta = fullDoby * S.dietaFull;
    const suf = fullDoby === 1 ? "a" : fullDoby < 5 ? "y" : "";
    let parts = [`${fullDoby} dob${suf} × 45 zł`];

    if (remH === 0) {
      // Dokładnie pełne doby — nic do dodania
    } else if (remH <= 8) {
      // Niepełna doba do 8h → ½ diety
      dieta += S.dietaHalf;
      parts.push(`${hDisp(remH)} → ½ diety`);
    } else {
      // Niepełna doba powyżej 8h → pełna dieta
      dieta += S.dietaFull;
      parts.push(`${hDisp(remH)} → pełna dieta`);
    }
    label = parts.join(" + ");
  }

  return { doby: Math.floor(totalH / 24), dieta, label, totalH };
}
...
function lockDelegKey(entryId, trainerId) { return `${delegKey(entryId, trainerId)}_lock`; }
function isDelegLocked(entryId, trainerId) { return localStorage.getItem(lockDelegKey(entryId, trainerId)) === "1"; }
function setDelegLock(entryId, trainerId, val) {
  if (val) localStorage.setItem(lockDelegKey(entryId, trainerId), "1");
  else localStorage.removeItem(lockDelegKey(entryId, trainerId));
}
...
export function DelegationForm({ entry, onClose }) {
  ...
  const [isLocked, setIsLocked] = useState(() => isDelegLocked(entry?.id, entry.trainer));
  function toggleLock() {
    const next = !isLocked;
    setIsLocked(next);
    setDelegLock(entry?.id, entry.trainer, next);
  }

  // Sekcja: Podróż
  const [tripPurpose, setTripPurpose]   = useState(delegCache?.tripPurpose ?? trainingTitle);
  ...
  useEffect(() => {
    async function fetchReport() {
      ...
      try {
        ...
        setReportData(d);
        // Jeśli raport serwisowy jest zablokowany — nie zaciągaj z niego danych do delegacji
        const srLockKey = `engel_report_${entry.trainer}_${entry.date}_lock`;
        if (localStorage.getItem(srLockKey) === "1") { setReportLoading(false); return; }
  ...
  // Sekcja: Zaliczka (w podsumowaniu)
  ...
  useEffect(() => {
    saveDelegData(entry?.id, entry.trainer, {
      tripPurpose, origin, destination, depDate, depTime, retDate, retTime,
      transport, km, ticketCost, ticketRoute, otherCost, otherDesc,
      nights, meals, extraCosts, payAdvance,
    });
  }, [tripPurpose, origin, destination, depDate, depTime, retDate, retTime,
      transport, km, ticketCost, ticketRoute, otherCost, otherDesc,
      nights, meals, extraCosts, payAdvance]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  // Karta firmowa = suma noclegów + inne koszty oznaczonych jako "card"
  // Ryczałt = należność pracownika (jak dieta) — nigdy nie jest płatnością kartą ani gotówką.
  // Liczy się w gross, przepływa do wypłaty dla pracownika. Tylko rachunki mają payMethod.
  const cardNights = nights.reduce((s, n) => {
    if (n.type === "ryczalt") return s;                      // ryczałt → pomiń
    if ((n.payMethod || "card") !== "card") return s;
    return s + Math.min(n.amount || 0, S.maxNocleg);
  }, 0);
  const cardExtras = extraCosts.reduce((s, x) =>
    (x.payMethod || "card") === "card" ? s + (parseFloat(x.amount) || 0) : s, 0);
  const card = cardNights + cardExtras;

  // Gotówka = suma noclegów + inne koszty oznaczonych jako "cash"
  const cashNights = nights.reduce((s, n) => {
    if (n.type === "ryczalt") return s;                      // ryczałt → pomiń
    if ((n.payMethod || "card") !== "cash") return s;
    return s + Math.min(n.amount || 0, S.maxNocleg);
  }, 0);
  const cashExtras = extraCosts.reduce((s, x) =>
    (x.payMethod || "card") === "cash" ? s + (parseFloat(x.amount) || 0) : s, 0);
  const cash = cashNights + cashExtras;

  const adv = parseFloat(payAdvance) || 0;
  const net = gross - card - adv;
  /* eslint-enable react-hooks/exhaustive-deps */

  /* ── Noclegi helpers ────────────────────────────────────────────────── */
  const addNight    = () => setNights(n => [...n, { type: "receipt", amount: 0, payMethod: "card", name: "Hotel" }]);
  ...
  const addExtra    = () => setExtraCosts(e => [...e, { desc: "", amount: "", payMethod: "card" }]);
  ...
  const setNightPayMethod = (i, m) => setNights(n => n.map((x, idx) => idx === i ? { ...x, payMethod: m } : x));
  const setNightName     = (i, v) => setNights(n => n.map((x, idx) => idx === i ? { ...x, name: v } : x));
  const setExtraPayMethod = (i, m) => setExtraCosts(e => e.map((x, idx) => idx === i ? { ...x, payMethod: m } : x));
  ...
  }, [trainerName, trainingTitle, tripPurpose, entry, origin, destination, depDate, depTime, retDate, retTime, transport, km, plate, ticketCost, ticketRoute, otherCost, otherDesc, nights, meals, extraCosts, extraTotal, payAdvance, card, cash, dur, dietaBase, mealCut, nightsTotal, transportCost, gross, net]);
  ...
  return createPortal(
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.55)", zIndex:1200, display:"flex", flexDirection:"column", fontFamily:FONT }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", maxWidth:520, margin:"0 auto", minHeight:"100%", background:C.greyBg }}>
        ...
        {/* ── Nagłówek ── */}
        <div style={{ background:C.black, padding:"14px 16px", paddingTop:"calc(14px + env(safe-area-inset-top))", display:"flex", alignItems:"center", justifyContent:"space-between", position:"sticky", top:0, zIndex:10 }}>
        ...
        <div style={card_}>
          ...
          <div style={secBody}>
            <div>
              <label style={lbl}>Cel podróży</label>
              <input style={inp} type="text" placeholder="np. Szkolenie PLASTPOL — obsługa maszyn"
                value={tripPurpose} onChange={e => setTripPurpose(e.target.value)} />
            </div>
            <div style={row2}>
              <div>
            ...
            <div style={row2}>
              <div>
                ...
                <DateInputDel value={depDate} onChange={setDepDate} style={inp} />
              </div>
              <div>
            ...
            <div style={row2}>
              <div>
                ...
                <DateInputDel value={retDate} onChange={setRetDate} style={inp} />
              </div>
              <div>
            ...
            <div>
              ...
              <div style={{ fontSize:10, color:C.greyMid, lineHeight:1.4, marginTop:4 }}>
                Obliczane automatycznie · 45 zł/dobę · do 8h: brak · 8–12h: ½ diety · pow. 12h: pełna · niepełna doba (do 8h): ½ diety
              </div>
            </div>
          </div>
        </div>
          ...
          <div style={secBody}>
            ...
            {nights.map((n, i) => (
              <div key={i} style={{ border:`1.5px solid ${C.grey}`, borderRadius:6, overflow:"hidden" }}>
                {/* ── Wiersz główny: nazwa | kwota | typ | usuń ── */}
                <div style={{ display:"flex", alignItems:"center", gap:6, padding:"8px 10px" }}>
                  {/* Nazwa (edytowalna, domyślnie "Hotel") */}
                  <input
                    type="text"
                    value={n.name ?? "Hotel"}
                    onChange={e => setNightName(i, e.target.value)}
                    placeholder="Hotel"
                    style={{ flex:1, padding:"7px 10px", border:`1.5px solid ${C.grey}`, borderRadius:6, fontSize:13, fontFamily:FONT, outline:"none", background:C.white }}
                  />
                  {/* Kwota — tylko dla rachunku */}
                  {n.type === "receipt" ? (
                    <input
                      type="number"
                      placeholder="zł"
                      value={n.amount > 0 ? n.amount : ""}
                      onChange={e => setNightAmount(i, e.target.value)}
                      style={{ width:90, padding:"7px 10px", border:`1.5px solid ${C.grey}`, borderRadius:6, fontSize:13, fontFamily:FONT, outline:"none", textAlign:"right" }}
                    />
                  ) : (
                    /* Ryczałt — kwota stała, nieaktywna */
                    <div style={{ width:90, padding:"7px 10px", borderRadius:6, fontSize:12, fontWeight:800, color:C.greenDk, background:"#F0F7E0", textAlign:"right", whiteSpace:"nowrap" }}>
                      {fmt(S.ryczNocleg)}
                    </div>
                  )}
                  {/* Toggle Ryczałt — kliknięcie przełącza między rachunek ↔ ryczałt */}
                  <button
                    onClick={() => setNightType(i, n.type === "ryczalt" ? "receipt" : "ryczalt")}
                    style={{ padding:"5px 10px", fontSize:10, fontWeight:700, border:`1.5px solid ${n.type === "ryczalt" ? C.green : C.grey}`, borderRadius:4, background: n.type === "ryczalt" ? "#F0F7E0" : C.white, color: n.type === "ryczalt" ? C.greenDk : C.greyDk, cursor:"pointer", fontFamily:FONT, whiteSpace:"nowrap", flexShrink:0 }}>
                    Ryczałt
                  </button>
                  <button onClick={() => removeNight(i)} style={{ background:"none", border:"none", fontSize:18, color:C.greyMid, cursor:"pointer", padding:"0 2px", flexShrink:0 }}>×</button>
                </div>
                {/* Karta / Gotówka — tylko dla rachunku */}
                {n.type !== "ryczalt" && (
                  <div style={{ display:"flex", gap:6, padding:"0 10px 8px" }}>
                    {[["card","💳 Karta firmowa","#EBF5FB",C.blue],["cash","💵 Gotówka","#FEF9F0",C.amber]].map(([val, lbl, bg, col]) => {
                      const active = (n.payMethod || "card") === val;
                      return (
                        <label key={val} onClick={() => setNightPayMethod(i, val)} style={{ display:"flex", alignItems:"center", gap:5, padding:"5px 10px", borderRadius:6, border:`1.5px solid ${active ? col : C.grey}`, background: active ? bg : C.white, fontSize:11, fontWeight:700, color: active ? col : C.greyMid, cursor:"pointer", userSelect:"none", flex:1, justifyContent:"center" }}>
                          <input type="radio" name={`nightPay_${i}`} value={val} checked={active} onChange={() => setNightPayMethod(i, val)} style={{ display:"none" }} />
                          {lbl}
                        </label>
                      );
                    })}
        ...
        <div style={card_}>
          ...
          <div style={secBody}>
            ...
            {extraCosts.map((x, i) => (
              <div key={i} style={{ border:`1.5px solid ${C.grey}`, borderRadius:6, overflow:"hidden", marginBottom:4 }}>
                <div style={{ display:"flex", alignItems:"center", gap:8, padding:"8px 10px" }}>
                  <input style={{ ...inp, flex:1, margin:0 }} type="text" placeholder="Opis kosztu" value={x.desc} onChange={e => setExtraDesc(i, e.target.value)} />
                  <input style={{ ...inp, width:90, textAlign:"right", margin:0 }} type="number" placeholder="0,00" value={x.amount} onChange={e => setExtraAmount(i, e.target.value)} />
                ...
                <div style={{ display:"flex", gap:6, padding:"0 10px 8px" }}>
                  {[["card","💳 Karta firmowa","#EBF5FB",C.blue],["cash","💵 Gotówka","#FEF9F0",C.amber]].map(([val, lbl, bg, col]) => {
                    const active = (x.payMethod || "card") === val;
                    return (
                      <label key={val} onClick={() => setExtraPayMethod(i, val)} style={{ display:"flex", alignItems:"center", gap:5, padding:"5px 10px", borderRadius:6, border:`1.5px solid ${active ? col : C.grey}`, background: active ? bg : C.white, fontSize:11, fontWeight:700, color: active ? col : C.greyMid, cursor:"pointer", userSelect:"none", flex:1, justifyContent:"center" }}>
                        <input type="radio" name={`extraPay_${i}`} value={val} checked={active} onChange={() => setExtraPayMethod(i, val)} style={{ display:"none" }} />
                        {lbl}
                      </label>
                    );
                  })}
        ...
        <div style={{ ...card_, border:`2px solid #F0F7E0`, marginBottom:0 }}>
          ...
          <div style={{ padding:"10px 14px" }}>
            ...
            {/* Karta firmowa (auto) */}
            {card > 0 && (
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"6px 0", borderBottom:`1px solid ${C.grey}`, fontSize:12 }}>
                <span style={{ color:C.blue }}>💳 Zapłacono kartą firmową</span>
                <span style={{ fontWeight:700, color:C.red }}>− {fmt(card)}</span>
              </div>
            )}
            {/* Gotówka (auto) */}
            {cash > 0 && (
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"6px 0", borderBottom:`1px solid ${C.grey}`, fontSize:12 }}>
                <span style={{ color:C.amber }}>💵 Zapłacono gotówką (zwrot)</span>
                <span style={{ fontWeight:700, color:C.greenDk }}>+ {fmt(cash)}</span>
              </div>
            )}
            {/* Zaliczka — pole do ręcznego wpisania */}
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"6px 0", borderBottom:`1px solid ${C.grey}`, fontSize:12, gap:8 }}>
              <span style={{ color:C.greenDk, whiteSpace:"nowrap" }}>💸 Zaliczka pobrana</span>
              <div style={{ display:"flex", alignItems:"center", gap:4 }}>
                <input
                  type="number" placeholder="0,00" value={payAdvance}
                  onChange={e => setPayAdvance(e.target.value)}
                  style={{ width:88, padding:"5px 8px", border:`1.5px solid ${C.grey}`, borderRadius:6, fontSize:12, color:C.black, fontFamily:FONT, outline:"none", textAlign:"right" }}
                />
                <span style={{ fontSize:11, color:C.greyMid }}>zł</span>
              </div>
            </div>
        ...
        <div style={{ padding:"10px 12px 32px", display:"flex", flexDirection:"column", gap:8 }}>
          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
            <button onClick={handleGenerate} disabled={saving} style={{ flex:1, padding:14, background: saving ? C.greyMid : C.black, color:"#fff", border:"none", borderRadius:8, fontSize:14, fontWeight:800, cursor: saving ? "default" : "pointer", fontFamily:FONT, display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
              {saving ? "⏳ Zapisuję…" : "📄 Generuj PDF rozliczenia"}
            </button>
            <button onClick={toggleLock} title={isLocked ? "Odblokuj rozliczenie" : "Zablokuj rozliczenie"} style={{ flexShrink:0, width:48, height:48, borderRadius:8, border:`2px solid ${isLocked ? "#E74C3C" : C.grey}`, background:isLocked ? "#E74C3C" : C.white, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", padding:0 }}>
