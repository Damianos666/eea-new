...
function calcDuration(depDate, depTime, retDate, retTime) {
  ...
  if (ret <= dep) {
    return {
      doby: 0,
      dieta: 0,
      label: "Data powrotu musi być późniejsza",
      totalH: 0,
    };
  }
...
// localStorage delegacji — TTL 2 dni
const DELEG_TTL = 2 * 24 * 60 * 60 * 1000;

function delegKey(entryId, trainerId) {
  return `engel_deleg_${trainerId}_${entryId}`;
}

function saveDelegData(entryId, trainerId, data) {
  try {
    localStorage.setItem(
      delegKey(entryId, trainerId),
      JSON.stringify({
        savedAt: Date.now(),
        data,
      })
...
function loadDelegData(entryId, trainerId) {
  try {
    ...
    if (Date.now() - savedAt > DELEG_TTL) {
      localStorage.removeItem(delegKey(entryId, trainerId));
      return null;
    }
...
function lockDelegKey(entryId, trainerId) {
  return `${delegKey(entryId, trainerId)}_lock`;
}

function isDelegLocked(entryId, trainerId) {
  return localStorage.getItem(lockDelegKey(entryId, trainerId)) === "1";
}
...
export function DelegationForm({ entry, onClose }) {
  ...
  const trainingTitle = entry.title || "Szkolenie wyjazdowe";
  ...
  const [plate, setPlate] = useState(() => {
    try {
      return localStorage.getItem("engel_plate") || "";
    } catch {
      return "";
    }
  });
    ...
    async function fetchPlate() {
      try {
        ...
        if (Array.isArray(data) && data.length && data[0].plate) {
          ...
          try {
            localStorage.setItem("engel_plate", data[0].plate);
          } catch {}
        }
      } catch (e) {
  ...
  function handlePlateChange(val) {
    ...
    try {
      localStorage.setItem("engel_plate", val);
    } catch {}
    ...
    plateSaveTimer.current = setTimeout(async () => {
      try {
        ...
        await db.upsert(
          ...
          "trainer_settings",
          {
            trainer_id: String(entry.trainer),
            plate: val,
          },
          "trainer_id"
        );
  ...
  useEffect(() => {
    async function fetchReport() {
      ...
      try {
        ...
        // Jeśli rozliczenie delegacji jest zablokowane — nie zaciągaj danych z raportu serwisowego
        if (isDelegLocked(entry?.id, entry.trainer)) {
          setReportLoading(false);
          return;
        }
        ...
        const totalKm = rows.reduce(
          (acc, r) => acc + (parseInt(r.km, 10) || 0) + (parseInt(r.retKm, 10) || 0),
          0
        );
  ...
  const nightsTotal = nights.reduce(
    (acc, n) => acc + (n.type === "ryczalt" ? S.ryczNocleg : Math.min(n.amount || 0, S.maxNocleg)),
    0
  );

  const transportCost =
    transport === "train"
      ? parseFloat(ticketCost) || 0
      : transport === "other"
  ...
  const cardNights = nights.reduce((s, n) => {
    if (n.type === "ryczalt") return s;
    if ((n.payMethod || "card") !== "card") return s;
    return s + Math.min(n.amount || 0, S.maxNocleg);
  }, 0);

  const cardExtras = extraCosts.reduce(
    (s, x) => ((x.payMethod || "card") === "card" ? s + (parseFloat(x.amount) || 0) : s),
    0
  );
  ...
  const cashExtras = extraCosts.reduce(
    (s, x) => ((x.payMethod || "card") === "cash" ? s + (parseFloat(x.amount) || 0) : s),
    0
  );
  ...
  const addNight = () =>
    setNights(n => [...n, { type: "receipt", amount: 0, payMethod: "card", name: "Hotel" }]);

  const removeNight = i => setNights(n => n.filter((_, idx) => idx !== i));

  const setNightType = (i, type) =>
    setNights(n =>
      n.map((x, idx) =>
        idx === i
          ? {
              ...x,
              type,
              amount: type === "ryczalt" ? 0 : x.amount,
            }
          : x
      )
    );

  const setNightAmount = (i, val) =>
    setNights(n =>
      n.map((x, idx) =>
        idx === i
          ? {
  ...
  const addExtra = () =>
    setExtraCosts(e => [...e, { desc: "", amount: "", payMethod: "card" }]);

  const removeExtra = i => setExtraCosts(e => e.filter((_, idx) => idx !== i));

  const setExtraDesc = (i, v) =>
    setExtraCosts(e =>
      e.map((x, idx) =>
        idx === i
          ? {
  ...
  const lockedFieldsStyle = isLocked
    ? {
        pointerEvents: "none",
        opacity: 0.65,
        userSelect: "none",
    ...
    return createPortal(
      ...
      >
        ...
        >
          ...
          >
            Rozliczenie zostało przesłane do systemu.
          </div>
      ...
      >
        ...
        >
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div>
                ...
                <div style={{ color: C.greyMid, fontSize: 11, marginTop: 1 }}>
                  {"Szkolenie wyjazdowe"}
                </div>
              </div>
            </div>
          ...
          {reportLoading && (
            ...
            >
              <span>🔄</span>
              <span>Sprawdzam raport serwisowy…</span>
            </div>
          )}
          ...
          {!reportLoading && reportStatus === "submitted" && reportData && (
            ...
            >
              ...
              <div>
                ...
                >
                  {"Dane zaciągnięte z raportu serwisowego"}
                </div>
          ...
          {!reportLoading && reportStatus === "other" && (
            ...
            >
              ...
              <div>
                ...
                >
                  Raport serwisowy nie jest jeszcze finalny
                </div>
                ...
                >
                  Dane zostały wstępnie zaciągnięte. Upewnij się że raport jest podpisany i zapisany.
                </div>
              </div>
            </div>
          )}
          ...
          {!reportLoading && reportStatus === "none" && (
            ...
            >
              ...
              <div>
                ...
                >
                  Brak raportu serwisowego w systemie
                </div>
                ...
                >
                  Możesz wypełnić delegację ręcznie lub wrócić po zapisaniu raportu.
                </div>
              </div>
            </div>
          )}
          ...
          {isLocked && (
            ...
            >
              🔒 Rozliczenie delegacji jest zablokowane — pola są tylko do odczytu.
              Możesz nadal wygenerować PDF, anulować lub odblokować rozliczenie.
            </div>
          )}
          ...
          <div style={lockedFieldsStyle}>
            ...
            <div style={card_}>
              ...
              <div style={secBody}>
                {[
                  ["Trener", trainerName],
                  ["Szkolenie wyjazdowe", trainingTitle],
                ].map(([k, v]) => (
                  <div
                    key={k}
                    style={{
                ...
                >
                  ...
                  >
                    {entry.date}
                    {entry.endDate && entry.endDate !== entry.date ? ` → ${entry.endDate}` : ""}
                  </span>
                </div>
              </div>
            </div>
              ...
              <div style={secBody}>
                ...
                <div>
                  ...
                  <input
                    ...
                    value={
                      dur.totalH > 0
                        ? `${dur.label} → base allowance: ${fmt(dietaBase)}`
                    ...
                    {[
                      ...
                      <button
                        ...
                        style={{
                          ...
                          fontWeight: 700,
                          border: "none",
                          borderRight: `1.5px solid ${C.grey}`,
                          background: transport === mode ? C.blue : C.white,
                          color: transport === mode ? "#fff" : C.greyDk,
                          cursor: "pointer",
                          fontFamily: FONT,
              ...
              <div style={secBody}>
                {nights.length === 0 && (
                  <div style={{ fontSize: 11, color: C.greyMid }}>
                    Dodaj noclegi jeśli były poniesione koszty
                  </div>
                )}
                ...
                {nights.map((n, i) => (
                  ...
                  >
                    ...
                    >
                      ...
                      <button
                        ...
                        style={{
                          ...
                          fontSize: 10,
                          fontWeight: 700,
                          border: `1.5px solid ${n.type === "ryczalt" ? C.green : C.grey}`,
                          borderRadius: 4,
                          background: n.type === "ryczalt" ? "#F0F7E0" : C.white,
                          color: n.type === "ryczalt" ? C.greenDk : C.greyDk,
                          cursor: "pointer",
                          fontFamily: FONT,
                          whiteSpace: "nowrap",
                          flexShrink: 0,
                    ...
                    {n.type !== "ryczalt" && (
                      ...
                      >
                        {[
                          ["card", "💳 Karta firmowa", "#EBF5FB", C.blue],
                          ["cash", "💵 Gotówka", "#FEF9F0", C.amber],
                        ].map(([val, label_, bg, col]) => {
                          const active = (n.payMethod || "card") === val;

                          return (
                            <label
                              key={val}
                              onClick={() => setNightPayMethod(i, val)}
                              style={{
                                display: "flex",
                                alignItems: "center",
                                gap: 5,
                                padding: "5px 10px",
                                borderRadius: 6,
                                border: `1.5px solid ${active ? col : C.grey}`,
                                background: active ? bg : C.white,
                                fontSize: 11,
                                fontWeight: 700,
                                color: active ? col : C.greyMid,
                                cursor: "pointer",
                                userSelect: "none",
                                flex: 1,
                                justifyContent: "center",
                            ...
                            >
                              <input
                                ...
                                value={val}
                                checked={active}
                                onChange={() => setNightPayMethod(i, val)}
                                style={{ display: "none" }}
                              />
                              {label_}
                            </label>
            ...
            <div style={card_}>
              ...
              <div style={secBody}>
                {[
                  ...
                  <div
                    ...
                    style={{
                      ...
                      gap: 12,
                      padding: "8px 10px",
                      border: `1.5px solid ${meals[idx] > 0 ? C.amber : C.grey}`,
                      borderRadius: 6,
                      background: meals[idx] > 0 ? "#FEF9F0" : C.white,
                    }}
                  >
                    <div>
                      <div
                ...
                >
                  Maks. odliczenie nie może przekroczyć kwoty diety za dobę.
                </div>
              </div>
            </div>
            ...
            <div style={card_}>
              ...
              <div style={secBody}>
                {extraCosts.length === 0 && (
                  <div style={{ fontSize: 11, color: C.greyMid }}>
                    Dodaj dodatkowe koszty (parking, opłaty, materiały…)
                  </div>
                )}
                  ...
                  >
                    ...
                    >
                      ...
                      <input
                        ...
                        placeholder="0,00"
                        value={x.amount}
                        onChange={e => setExtraAmount(i, e.target.value)}
                      />
                    ...
                    >
                      {[
                        ...
                        return (
                          <label
                            key={val}
                            onClick={() => setExtraPayMethod(i, val)}
                            style={{
                              display: "flex",
                              alignItems: "center",
                              gap: 5,
                          ...
                          >
                            <input
                              ...
                              value={val}
                              checked={active}
                              onChange={() => setExtraPayMethod(i, val)}
                              style={{ display: "none" }}
                            />
                            {label_}
                          </label>
            ...
            >
              ...
              <div style={{ padding: "10px 14px" }}>
                {[
                  {
                    k: dur.label ? `Dieta (${dur.label})` : `Dieta (${dur.doby} dób)`,
                    v: `+ ${fmt(dietaBase)}`,
                    red: false,
                  },
                  {
                    k: "Pomniejszenia za posiłki",
                    v: mealCut > 0 ? `− ${fmt(mealCut)}` : "0,00 zł",
                    red: mealCut > 0,
                  },
                  {
                    k: "Noclegi",
                    v: nightsTotal > 0 ? `+ ${fmt(nightsTotal)}` : "0,00 zł",
                    red: false,
                  },
                  {
                    k: "Transport",
                    v: transportCost > 0 ? `+ ${fmt(transportCost)}` : "0,00 zł",
                    red: false,
                  },
                  {
                    k: "Inne koszty",
                    v: extraTotal > 0 ? `+ ${fmt(extraTotal)}` : "0,00 zł",
                    red: false,
                  },
                ].map(row => (
                  <div
                ...
                >
                  <span style={{ color: C.greenDk, whiteSpace: "nowrap" }}>
                    💸 Zaliczka pobrana
                  </span>
                ...
                {transport === "car" && km && (
                  ...
                  >
                    📍 {"Przebieg pojazdu"}: <strong>{km}</strong> km — {"km — ewidencja pojazdu"}
                  </div>
                )}
              </div>
            </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <button
                ...
                style={{
                  flex: 1,
                  padding: 14,
                  background: saving ? C.greyMid : C.black,
                  color: "#fff",
                  border: "none",
                  borderRadius: 8,
                  fontSize: 14,
              ...
              <button
                onClick={toggleLock}
                title={isLocked ? "Odblokuj rozliczenie" : "Zablokuj rozliczenie"}
                style={{
                  flexShrink: 0,
                  width: 48,
                  height: 48,
                  borderRadius: 8,
                  border: `2px solid ${isLocked ? "#E74C3C" : C.grey}`,
                  background: isLocked ? "#E74C3C" : C.white,
                  cursor: "pointer",
                  display: "flex",
                  alignItems: "center",
                  justifyContent: "center",
