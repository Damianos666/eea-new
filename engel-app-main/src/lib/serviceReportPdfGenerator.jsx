...
export async function generateServiceReportPdf(data) {
  ...
  const fileName = `raport_serwisowy_${safeName}_${safeDate}.pdf`;
  const url = URL.createObjectURL(blob);

  // Na iOS Safari atrybut `download` nie działa — przeglądarka nawiguje
  // całą kartę do blob: URL, przez co React traci stan i użytkownik
  // zostaje wylogowany po zamknięciu podglądu PDF.
  // Rozwiązanie: window.open otwiera PDF w NOWEJ karcie, nie ruszając
  // stanu aplikacji. Revoke z opóźnieniem, żeby karta zdążyła załadować zasób.
  const win = window.open(url, "_blank");
  if (!win || win.closed) {
    // Fallback gdy popup jest zablokowany (np. PWA standalone na iOS)
