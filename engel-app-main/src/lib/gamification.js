...
/* ─── ENGEL Expert Titles — tytuły za ukończenie pełnej kategorii ─────────── */

/**
 * Definicja tytułów eksperckich.
 * Każdy tytuł wymaga zaliczenia WSZYSTKICH szkoleń ze swojej grupy.
 * Master wymaga zdobycia wszystkich trzech tytułów eksperckich.
 */
export const EXPERT_TITLES = [
  {
    id:       "tech",
    group:    "tech",
    label:    "ENGEL Process Expert",
    badge:    "🔬",
    color:    "#3B6D11",
    colorBg:  "#EAF3DE",
    colorBorder: "#BDDBA0",
    trainings: ["PIM","PMC","PPS","PIQ","PPC","PMS","PQC"],
    desc:     "Wszystkie szkolenia technologiczne zaliczone",
  },
  {
    id:       "ur",
    group:    "ur",
    label:    "ENGEL Maintenance Expert",
    badge:    "⚙️",
    color:    "#854F0B",
    colorBg:  "#FAEEDA",
    colorBorder: "#F0C070",
    trainings: ["MTO","MTH-L1","MTH-L2-victory","MTH-L2-duo","MTE-L1","MTE-L2-victory","MTE-L2-duo","MTE-L2-em","MTM","RTM-viper"],
    desc:     "Wszystkie szkolenia utrzymania ruchu zaliczone",
  },
  {
    id:       "maszyny",
    group:    "maszyny",
    label:    "ENGEL Operation Expert",
    badge:    "🎯",
    color:    "#A32D2D",
    colorBg:  "#FCEBEB",
    colorBorder: "#F0A0A0",
    trainings: ["MO-L1","MO-L2","RO-B","RO-L1-easix","RO-L2-easix","RO-L1-viper","RO-L2-viper"],
    desc:     "Wszystkie szkolenia obsługi maszyn zaliczone",
  },
];
...
export const MASTER_TITLE = {
  ...
  colorBg:  "#F0EAFE",
  colorBorder: "#C9B8F0",
  desc:     "Wszystkie trzy tytuły eksperckie zdobyte",
};

/**
 * Oblicza postęp i status dla każdego tytułu eksperckiego
 * oraz finalnego tytułu Master.
 *
 * @param {Array} completed  - tablica zaliczeń z App.jsx (każde ma .training.id)
 * @returns {{ titles: Array, master: Object }}
 *
 * Każdy element titles:
 *   { ...EXPERT_TITLES[i], done: number, total: number, earned: boolean, pct: number }
 * master:
 *   { ...MASTER_TITLE, earned: boolean, earnedCount: number }
 */
export function getExpertProgress(completed) {
  const completedIds = new Set(
    (completed || []).map(c => c?.training?.id).filter(Boolean)
  );

  const titles = EXPERT_TITLES.map(t => {
    const done  = t.trainings.filter(id => completedIds.has(id)).length;
    const total = t.trainings.length;
    return {
      ...t,
      done,
      total,
  ...
  const master = {
    ...MASTER_TITLE,
    earned:      earnedCount === EXPERT_TITLES.length,
    earnedCount,
    total:       EXPERT_TITLES.length,
  };

  return { titles, master };
}
